...
This slice does not implement Telegram approval commands, email or power-control commands. Conversational assent never changes spending, access or reserved-action permissions. These remain separate parent-spec tickets. Protect the company data directory and backups as private; credentials stay in service-only files and never enter model context or transport error logs.
