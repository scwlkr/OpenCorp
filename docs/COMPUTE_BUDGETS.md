...
Management writes the recommendation in Markdown and emails it through `owner.propose`. Add `computeGrant` with `ceilingMicrousd` (integer millionths of one USD), existing `assignmentIds`, and exact `routes` objects containing `provider` and `model`. Use `channel:"email"`, a future `expiresAt`, and no `actionId`. Include expected result, time/cost estimate basis, uncertainty and a free alternative in the prose. The service adds the total ceiling, exact routes and frozen named work to the email. Only the existing authenticated, proposal-specific Owner reply approves it. Generic chat, policy updates and employee commands cannot authorize it.
