...
function setup(){const root=mkdtempSync(join(tmpdir(),'opencorp-api-'));roots.push(root);const store=new CompanyStore(root);stores.push(store);store.bootstrap();const pause=vi.fn(async()=>{}),start=vi.fn(),configureResources=vi.fn(async()=>{});const scheduler=Object.assign(new Scheduler(store,{configureResources:vi.fn(async()=>{})} as unknown as LocalRuntime,{} as CorporateBroker,'http://localhost'),{pause,start,configureResources});const installMicroModels=vi.fn(async(_ids?:unknown)=>[]);const runtime={installMicroModels,status:()=>({started:false,activeRuns:[],ollama:{running:false},inferenceSlots:1})} as unknown as LocalRuntime;const broker=new CorporateBroker(store,root);const app=ownerApp({store,runtime,broker,scheduler,getUrl:()=> 'http://127.0.0.1:4310'});const token=readFileSync(join(root,'owner-token'),'utf8');const request=(path:string,init:RequestInit={})=>app.request(`http://127.0.0.1:4310${path}`,{...init,headers:{host:'127.0.0.1:4310',authorization:`Bearer ${token}`,...init.headers}});return {runtime,store,broker,app,request,pause,start,configureResources,root,installMicroModels,scheduler};}
...
it('serializes mode changes, persists Stop before cancellation, and retains the power choice on resume',async()=>{
 const {request,store,pause,start,scheduler}=setup();let release!:()=>void;
 pause.mockImplementationOnce(()=>new Promise<void>(resolve=>{release=resolve;}));
 const control=(action:string)=>request('/api/v1/control',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action})});
 const stop=control('stop');await vi.waitFor(()=>expect(store.company.state).toBe('stopped'));
 const low=control('low');await new Promise(resolve=>setImmediate(resolve));
 expect(store.company.state).toBe('stopped');expect(start).not.toHaveBeenCalled();
 release();expect((await stop).status).toBe(200);expect((await low).status).toBe(200);
 expect(store.company).toMatchObject({state:'running',powerMode:'low'});
 expect(scheduler.runtime.configureResources).toHaveBeenLastCalledWith(expect.objectContaining({maxConcurrentTurns:1,maxProductiveTurns:1,maxSocialTurns:1,maxLoadedModels:1}));
 await control('stop');await control('resume');expect(store.company.powerMode).toBe('low');
 await control('full');expect(store.company).toMatchObject({state:'running',powerMode:'full'});
 expect(scheduler.runtime.configureResources).toHaveBeenLastCalledWith(expect.objectContaining({maxConcurrentTurns:store.policy.maxInference}));
});
