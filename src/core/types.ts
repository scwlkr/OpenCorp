...
export interface Employee extends RecordBase { name: string; badge: string; status: 'active' | 'dismissed'; positionId: string; departmentId: string | null; homeManagerId: string | null; modelId: string; fallbackModelIds?: string[]; role: string; roleVersion: number }
...
export interface Assignment extends RecordBase { projectId: string | null; employeeId: string; supervisorId: string; title: string; instructions: string; acceptance: string[]; dependencies: string[]; status: AssignmentState; priority: number; attempts: number; corrections: number; kind: string; availableAt: string; dataClass?: 'public'|'internal'|'confidential' }
