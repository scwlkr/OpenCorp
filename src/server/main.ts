...
const runtime=new LocalRuntime({dataRoot});const broker=new CorporateBroker(store,dataRoot);let workerHost='';
...
const maintain=setInterval(()=>{try{store.vault.scan();for(const name of ['daemon.log','service.log']){const path=join(dataRoot,'logs',name);if(existsSync(path)&&statSync(path).size>10_000_000){copyFileSync(path,`${path}.1`);truncateSync(path,0);}}}catch(error){store.emit('maintenance.error',{error:redact(String(error))});}},30_000);maintain.unref();
let ending=false;async function shutdown(){if(ending)return;ending=true;clearInterval(maintain);const deadline=setTimeout(()=>process.exit(1),7000);deadline.unref();try{await scheduler.shutdown();if('closeAllConnections' in server)server.closeAllConnections();if('closeAllConnections' in worker)worker.closeAllConnections();server.close();worker.close();store.close();if(existsSync(lockPath)&&JSON.parse(readFileSync(lockPath,'utf8')).pid===process.pid)unlinkSync(lockPath);}finally{clearTimeout(deadline);process.exit(0);}}
