...
export function resourceLimits(options: RuntimeOptions['resourceBudget'] = {}) {
  ...
  for (const key of ['maxProductiveTurns', 'maxConcurrentTurns', 'maxSocialTurns', 'maxLoadedModels'] as const) {
    if (!Number.isSafeInteger(limits[key]) || limits[key] < 1) throw new Error(`Invalid resource limit: ${key}`);
  }
  if (limits.maxProductiveTurns > limits.maxConcurrentTurns) throw new Error('Productive capacity exceeds inference slots');
...
export class ResourceBudget {
  ...
  private readonly contextReservations = new Map<string, number>();
  private readonly active = new Map<string, { model: LocalModel; workload: 'productive' | 'social' }>();
  ...
  admit(runId: string, model: LocalModel, workload: 'productive' | 'social' = 'productive', residentBytes = 0, pool?: 'primary' | 'micro'): () => void {
    ...
    if (workload === 'productive' && productive.length >= this.limits.maxProductiveTurns) throw new ResourceAdmissionError('Local productive slots occupied');
    if ((this.limits.productiveRemoteModelId || this.limits.productiveRemoteProfiles) && workload === 'productive' && productive.length) throw new ResourceAdmissionError('Retained provider capacity permits only one local productive turn');
    const memory = this.memory();
    const available = memory.available ?? memory.free;
    if (!entries.length) this.admittedFreeMemory = available;
    const contextBytes = (item: LocalModel) => item.contextMemoryBytes ?? item.contextTokens * 65536 + 256 * 1024 ** 2;
    if (model.contextMemoryBytes !== undefined && (!Number.isSafeInteger(model.contextMemoryBytes) || model.contextMemoryBytes <= 0)) throw new Error('Invalid context memory estimate');
    // Ollama allocates KV for every configured parallel slot when loading a model.
    const contexts = localParallel(this.limits, primary ? 'primary' : 'micro');
    const previousContext = this.contextReservations.get(model.artifactIdentity) ?? 0;
    const contextAllocation = Math.max(0, contexts * contextBytes(model) - previousContext);
    const contextEstimate = [...this.contextReservations.values()].reduce((sum, bytes) => sum + bytes, 0) + contextAllocation;
    ...
    const pendingAllocation = [...this.weightReservations.values()].reduce((sum, bytes) => sum + bytes, 0)
      + [...this.contextReservations.values()].reduce((sum, bytes) => sum + bytes, 0)
      + newAllocation;
    // An observation of an already-running model remains true even if this turn is refused.
    if (previousContext > 0) this.contextReservations.set(model.artifactIdentity, previousContext + contextAllocation);
    ...
    this.contextReservations.set(model.artifactIdentity, previousContext + contextAllocation);
    this.active.set(runId, { model, workload });
    let released = false;
    return () => {
      if (released) return; released = true;
      this.active.delete(runId);
      if (![...this.active.values()].some(entry => entry.model.artifactIdentity === model.artifactIdentity)) { this.weightReservations.delete(model.artifactIdentity); this.contextReservations.delete(model.artifactIdentity); }
    };
  }
...
/** Small workers use productive capacity too; social permission remains a run property. */
export function localParallel(limits: ReturnType<typeof resourceLimits>, pool: 'primary' | 'micro'): number {
  return pool === 'micro' ? Math.min(limits.maxConcurrentTurns, Math.max(limits.maxProductiveTurns, limits.maxSocialTurns))
    : limits.productiveRemoteModelId || limits.productiveRemoteProfiles ? 1 : limits.maxProductiveTurns;
}
