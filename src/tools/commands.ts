...
export const commandProperties:Record<string,any>={
 ...
 computeGrant:{type:'object',description:'Optional scoped paid text compute: ceilingMicrousd (integer USD millionths), assignmentIds (exact existing work), routes [{provider:"openai",model:exact snapshot}]. Requires email; cannot combine with actionId.',properties:{ceilingMicrousd:{type:'integer',minimum:1},assignmentIds:strings,routes:{type:'array',items:{type:'object',properties:{provider:{type:'string',enum:['openai']},model:{type:'string'}},required:['provider','model']}}},required:['ceilingMicrousd','assignmentIds','routes']},
