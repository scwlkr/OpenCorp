...
export const commandFields:Record<string,{required:string[];optional:string[]}>= {
 'owner.propose':{required:['title','content','proposalScope','expiresAt','channel'],optional:['actionId','computeGrant']},
 ...
 'product.register':{required:['name','repository','rationale'],optional:[]},
...
export const commandProperties:Record<string,any>={
 ...
 computeGrant:{type:'object',description:'Scoped paid text compute: USD-micro ceiling, exact assignments and routes. Email only; no actionId.',properties:{ceilingMicrousd:{type:'integer',minimum:1},assignmentIds:strings,routes:{type:'array',items:{type:'object',properties:{provider:{type:'string',enum:['openai']},model:{type:'string',description:'Snapshot only, e.g. gpt-4.1-mini-2025-04-14; no provider prefix.'},maxOutputTokens:{type:'integer',minimum:1,maximum:32768,description:'Per-request output ceiling; defaults to 1024.'}},required:['provider','model']}}},required:['ceilingMicrousd','assignmentIds','routes']},
