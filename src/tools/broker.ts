...
export const brokerTools=[
 ...
 {name:'compute_request',description:'Request supplemental paid text inference for this exact assignment under an approved compute proposal. Uses a stable dedupeKey; uncertain effects are never resent. No default paid route or new data access.',inputSchema:objectSchema({proposalId:string,provider:{type:'string',enum:['openai']},model:string,prompt:string,dedupeKey:string,maxOutputTokens:{type:'integer',minimum:1,maximum:32768}},['proposalId','provider','model','prompt','dedupeKey'])},
