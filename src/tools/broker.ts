...
Product setup: product.register_internal {name,verificationCommand,rationale} creates local software; product.register {name,repository,rationale} binds a permitted repository. Reviewed .opencorp/product.json supplies verificationCommand and release {target:github-release,command,assets}. Use a project, employee implementation, commit_work, verify_product and independent review_work. adopt_internal_tool {artifactId,entrypoint,employeeIds} exposes the reviewed version; another employee use_internal_tool {productId,args} performs actual isolated execution. Use existing verified dependency preparation; no credentials or new access.
...
For a merged product, prepare_release({artifactId,version,notes}) executes its configured release procedure (retained WalkLang fallback) and preserves immutable checked assets; publish_release({releaseId}) publishes through the connected identity with separate durable provider receipts. Choose the actual next appropriate version from the live release history. Other product release channels require their actual provider/candidate requirements.
...
export const brokerTools=[
 ...
 {name:'finish_assignment',description:'Complete implementation using independently reviewed artifact/delivery evidence and declared criteria.',inputSchema:objectSchema({assignmentId:string,artifactId:string,rationale:string},['assignmentId','artifactId','rationale'])},
 ...
 {name:'company_command',description:'Apply a corporate command; put required fields beside command.type. Call company_help for exact syntax. executive.appoint is a decision kind. Existing authority/state checks apply; company_detail exposes full records.',inputSchema:objectSchema({command:commandSchema},['command'])},
 ...
 {name:'prepare_release',description:'Package configured assets from exact reviewed, merged source in the sandbox.',inputSchema:objectSchema({artifactId:string,version:string,notes:string},['artifactId','version','notes'])},
...
Record the next accountable action with company_command {command:{type:"responsibility.update",assignmentId:"${context.original.id}",kind:your chosen continuation kind,action:your concrete next action,nextCheckAt:a future ISO timestamp within thirty days}}. Kinds: changed_approach, specialist_help, reassignment, prerequisite_work, scheduled_recheck, owner_decision. ownerId defaults to the original supervisor; changing it requires existing management authority. Include followupAssignmentId only for distinct nonterminal work, not a completed correction. For indispensable Owner input use owner.request linked to the original; an owner_decision continuation requires its open attentionId. Use assignment.update only within existing authority and acceptance checks. A continuation never completes original acceptance. After retaining the required action, summarize briefly and end.`;
 ...
 toolsFor(actor:Actor){
  ...
  return tools.map(tool=>{
   ...
   return {...tool,description:`Fields beside command.type: ${required}. company_help: schema. Authority applies.`,inputSchema:objectSchema({command:{type:'object',properties:{type:{type:'string',enum:types},...properties},required:['type'],additionalProperties:true}},['command'])};
  });
 }
 ...
 async call(actor:Actor,name:string,args:any):Promise<any>{
  ...
  try{
   ...
   case 'finish_assignment':{const original=this.store.need('assignments',args.assignmentId);if(original.kind!=='implementation')throw new DomainError('implementation_required','finish_assignment only completes original implementation work. For management work, perform the requested corporate actions, summarize actual results and end the turn; the scheduler handles completion. Do not add implementation requirements or repeat delivered effects.');if(!original.completionRequirements?.length)throw new DomainError('requirements_required','Management must declare full completionRequirements through assignment.update first; completionSource is supported only when creating an implementation assignment.');result=this.store.command(actor,{type:'assignment.update',assignmentId:original.id,status:'completed',rationale:args.rationale,completionEvidence:original.completionRequirements.map((r:any)=>({criterion:r.criterion,rationale:args.rationale,sources:[{type:r.source,id:args.artifactId}]}))});break;}
