...
export const brokerTools=[
 ...
 {name:'send_message',description:'Send authored content to recipientId. Optional projectId is a real project, never a requisition. Omit it for recruitment handoffs.',inputSchema:objectSchema({recipientId:{type:'string',minLength:1},content:{type:'string',minLength:1},projectId:{type:'string',minLength:1}},['recipientId','content'])},
 ...
 {name:'update_role',description:'Record full authored role content, truthful source and rationale for an employee you manage. Preserves identity and role history.',inputSchema:objectSchema({employeeId:string,content:string,source:string,rationale:string},['employeeId','content','source','rationale'])},
...
Record the next accountable action with company_command {command:{type:"responsibility.update",assignmentId:"${context.original.id}",kind:your chosen continuation kind,action:your concrete next action,nextCheckAt:a future ISO timestamp within thirty days}}. Kinds: changed_approach, specialist_help, reassignment, prerequisite_work, scheduled_recheck, owner_decision. ownerId defaults to the original supervisor; changing it requires existing management authority. Include followupAssignmentId only for distinct nonterminal work, not a completed correction. For indispensable Owner input use owner.request linked to the original; an owner_decision continuation requires its open attentionId. Use assignment.update only within existing authority and acceptance checks. A continuation never completes original acceptance. After retaining the required action, summarize briefly and end.`;
 ...
 private modelRoutingGuidance(state:ReturnType<CorporateBroker['readState']>){
  ...
  return {maxProductiveTurns:policy.maxProductiveTurns,qualifiedConcurrentModelIds,guidance:'These exact currently registered profiles qualify for shared productive capacity under current Owner policy; employee/project exclusions, provider caps, cooldowns and fresh runtime eligibility still apply. Other available authorized models remain valid solo choices, but may wait until all other productive runs finish. Selecting an available model does not reserve a slot or qualify new concurrency. Choose based on actual failure evidence and this scheduling tradeoff; do not change a model merely to claim a retry.'};
 }
 ...
 provisionPrompt(actor:Actor){
  ...
  const prompt=JSON.stringify({assignment:{id:assignment.id,title:assignment.title,instructions:text(assignment.instructions,700),acceptance:text(assignment.acceptance,500)},role:{version:employee.roleVersion,content:text(employee.role,500)},
 ...
 promptContext(actor:Actor){const {assignment}=this.context(actor),fault=this.faultPromptContext(actor);if(fault)return fault;const responsibility=this.responsibilityPromptContext(actor);if(responsibility)return responsibility;if(assignment.schedulerKey?.startsWith('formation:'))return this.formationContext(actor,assignment);const state=this.companyRead(actor,{collection:'summary'},true);state.assignment=brief(assignment);if(assignment.payload?.decisionId){const decision=this.readState(actor).decisions.find(d=>d.id===assignment.payload.decisionId);if(decision)state.assignedDecision=brief(decision);}return fitReadPages(state);}
 knowledgeContext(actor:Actor,scopeIds:string[],budgetChars=4500){
  ...
  const relevant=applicable.filter(k=>!superseded.has(k.id)).sort((a,b)=>Number(sourceLinked(b))-Number(sourceLinked(a))||(b.updatedAt??b.createdAt).localeCompare(a.updatedAt??a.createdAt));
