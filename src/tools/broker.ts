...
message.send {recipientId?,projectId?,content,wake?}; experience.record {summary,source,learned,environment?}; knowledge.write {scope:'company'|'products'|'departments'|'projects'|'employees',scopeId?,title,content,source}; role.update {employeeId,content,source,rationale}.
...
export const brokerTools=[
 ...
 {name:'send_message',description:'Message a colleague; wake:false is informational. projectId is a project, not a requisition.',inputSchema:objectSchema({recipientId:{type:'string',minLength:1},content:{type:'string',minLength:1},projectId:{type:'string',minLength:1},wake:{type:'boolean'}},['recipientId','content'])},
 ...
 async call(actor:Actor,name:string,args:any):Promise<any>{
  ...
  try{
   ...
   case 'send_message':{
    ...
    const fields=['recipientId','content','projectId','wake'];
