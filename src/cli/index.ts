...
export function createProgram(): Command {
  ...
  for (const name of ['products', 'projects', 'employees', 'decisions', 'attention', 'models', 'integrations', 'logs'] as const) program.command(name).description(`List company ${name} from the service`).action(async () => show(await client().request(name)));
