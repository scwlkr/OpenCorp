...
export class CompanyStore {
  ...
  private executeCommand(actor: Actor, c: CorporateCommand): any {
    ...
    switch (c.type) {
      ...
      case 'policy.update': {
        ...
        for (const [key,max] of [['maxLoadedModels',Number.MAX_SAFE_INTEGER],['maxProductiveTurns',Number.MAX_SAFE_INTEGER],['maxInference',Number.MAX_SAFE_INTEGER],['nativeJobs',1],['maxRetries',1],['maxCorrections',2],['reassessMinutes',1440]] as const) if (c[key] !== undefined) { if (!Number.isInteger(c[key]) || c[key] < (key === 'maxRetries' ? 0 : 1) || c[key] > max) throw new DomainError('invalid_limit',`${key} is outside permitted release limits`); patch[key] = c[key]; }
        ...
        const productiveQualification=c.productiveConcurrencyQualification===null?undefined:patch.productiveConcurrencyQualification??this.policy.productiveConcurrencyQualification;
        const productiveLimit=patch.maxProductiveTurns??this.policy.maxProductiveTurns??1;
        if(productiveLimit>(patch.maxInference??this.policy.maxInference))throw new DomainError('invalid_limit','Productive capacity exceeds inference slots');
        if(productiveQualification?.mode==='local-remotes'&&productiveLimit>productiveQualification.stableMaxProductiveTurns)throw new DomainError('invalid_limit','Productive capacity exceeds retained provider limits');
  ...
  claimNext(options: {workspace?: string; leaseMs?: number; assignmentId?: string; modelId?: string} = {}): EmployeeRun | undefined {
    return this.db.transaction(() => {
      ...
      const sharing=assignment.kind!=='social'&&productiveSharingAllowed(selected(modelId),productive.map(r=>selected(r.modelId)),{maxProductiveTurns:this.policy.maxProductiveTurns,productiveArtifactIdentity:qualification?.artifactIdentity,...(qualification?.mode==='local-remote'?{productiveRemoteModelId:qualification.remoteModelId,productiveRemoteArtifactIdentity:qualification.remoteArtifactIdentity}:qualification?.mode==='local-remotes'?{productiveRemoteProfiles:qualification.remoteProfiles,productiveProviderCaps:qualification.providerCaps}:{})});
